...
pipeline {
    ...
    environment {
        PROJECT_PATH = "C:\\Games\\Slot-CashMachine"
    }
            ...
            steps {
                script {
                    dir("${PROJECT_PATH}") {
                        ...
                        git clone git@github.com:DingDingHouse/Slot-CashMachine.git C:\\Games\\Slot-CashMachine || echo "Repository already exists, pulling latest changes."
                        cd Slot-CashMachine
                        git checkout main
                        git fetch --all
